...
export class AuthService {
    ...
    async register(payload: RegisterUserDTO): Promise<ResponseInterface> {
        ...
        if (!foundUser) {
            throw new NotFoundException({ message: 'NIP or email not found', status: false });
        }
        if (foundUser.isActive) {
            throw new ForbiddenException({ message: 'User is already registered', status: false });
        }

        foundUser.isActive = true;
        foundUser.password = await this.bcryptService.createHashPassword(payload.password);
        foundUser.isPasswordChanged = false;

        const role = await this.roleRepo.findOne({ where: { name: 'Employee' } });
        if (!role) {
            throw new BadRequestException({ message: 'Role Employee not found', status: false });
        }
    ...
    async login(payload: LoginUserDTO, res: Response): Promise<any> {
        try {
            const foundUser = await this.userRepo
                ...
                .where('(user.isActive = true AND user.email = :identifier) OR (user.isActive = true AND user.nip = :identifier)', {
                    identifier: payload.identifier,
                })
                .getOne();

            if (!foundUser) {
                throw new NotFoundException({ message: 'Email or password not found', status: false });
            }

            const passwordMatch = await this.bcryptService.checkPassword(payload.password, foundUser.password);
            if (!passwordMatch) {
                throw new BadRequestException({ message: 'Email or password is wrong', status: false });
            }

            const roleIds = foundUser.roles.map(ur => ur.role.id);
            const roles = foundUser.roles.map(ur => ({ id: ur.role.id, name: ur.role.name }));
            const menus = await this.menusService.findMenusByRole(roleIds);

            const tokenLogin = crypto.randomBytes(15).toString('hex').substring(0, 20);
            ...
            throw new InternalServerErrorException({ message: 'An error occurred during login', status: false });
        }
    }
}
